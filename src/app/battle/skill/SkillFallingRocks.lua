...
--]]
 ...
 SkillFallingRocks = class("SkillFallingRocks",function()
	return display.newLayer()
end)

function SkillFallingRocks:ctor(params,emenyCamp,callback,obj)
...
function SkillFallingRocks:init()
	self:createFalling()
 	self:runEndSkillAction()
end

function SkillFallingRocks:createPos(arry,index,beginPos,targetPos)
	arry[index] = {}
...
function SkillFallingRocks:createFalling()
...
--碰撞检测
--返回值(无)
function SkillFallingRocks:checkCollision()
...
function SkillFallingRocks:createFalling2()
	local bgMapSize = BattleLogic:getInstance():getBgMapSize()
	local beginY = bgMapSize.height
	local beginX = 10
	local midY = bgMapSize.height/2 - 200
...
--运行结束技能ACTION
--返回值(无)
function SkillFallingRocks:runEndSkillAction()
...
--删除技能
--返回值(无)
function SkillFallingRocks:removeSkill()
...
function SkillFallingRocks:onEnter()
 	
end

function SkillFallingRocks:onExit()
	
end

function SkillFallingRocks:onDestroy()
