...
--构造
--返回值(无)
function TouchLayer:ctor(mapPath)
...
function TouchLayer:init()
	self.bgMap = display.newSprite(self.mapPath)
...
--返回值(无)
function TouchLayer:setTouchAble(able,node)
...
--获取地图
--返回值(地图)
function TouchLayer:getBgMap()
	return self.bgMap
end

--获取地图当前放大倍数
--返回值(地图放大倍数)
function TouchLayer:getMapScale()
	return self.bgScale
end

--获取地图放大后的大小
--返回值(放大后的地图大小)
function TouchLayer:getBgMapSize()
...
--获取地图放大之前的原始大小
--返回值(地图放大前的大小)
function TouchLayer:getBgMapOldSize()
	return self.bgMapSize
end

--获取地图位置
--返回值(地图坐标)
function TouchLayer:getBgMapPos()
...
--pos 坐标
--返回值(无)
function TouchLayer:setBgMapPos(pos)
	self:getBgMap():setPosition(pos)
end

--开始滚动
--返回值(无)
function TouchLayer:beginScrollingTime()
...
--停止滚动
--返回值(无)
function TouchLayer:stopScrollingTime()
	if self.handle ~= nil then
...
--计算地图滑动值
--返回值(滑动后的坐标)
function TouchLayer:changeVecScrollDis(point,factorX,factorY)
...
--event 事件类型
--id 触摸点id
function TouchLayer:getTouchPos(event,id)
...
--event 事件
--返回值(拖动距离)
function TouchLayer:getDiff(event)
...
--event 事件类型
--返回值(无)
function TouchLayer:touchBegin(event)
...
--event 事件
--返回值(无)
function TouchLayer:touchAdd(event)
    self.touchCount = self.touchCount + 1
...
--event 触摸事件
--返回值(无)
function TouchLayer:touchRemove(event)
	self.touchCount = self.touchCount - 1
...
--event 事件
--返回值(无)
function TouchLayer:touchMove(event)
	if self.touchCount  > 1 then	
		self:stopScrollingTime()	 
...
--返回值(无)
function TouchLayer:setIsDrap(isDrap)
	self.isDrap = isDrap
end
...
function TouchLayer:isCanMovePos(dis)
...
--event 事件
--返回值(无)
function TouchLayer:dropMap(event)
...
-- scale 放大倍数
--返回值(无)
function TouchLayer:adjustMapSizeAndPos(focusPos,scale)
...
--event 事件
--返回值(放大倍数,焦点坐标)
function TouchLayer:getMapScaleAndFouce(event)
	local pointSet = {}
    for id, point in pairs(event.points) do
...
--event 事件
--返回值(无)
function TouchLayer:changeMapSize(event)
...
--event 事件
--返回值(无)
function TouchLayer:touchEnd(event)
	self.touchCount = 0
end

--是否拖拽了地图
--返回值(true:是,false:否)
function TouchLayer:isDrapMap()
	return self.isDrap
end
...
--inPos 地图坐标
--返回值(纠正后的地图坐标)
function TouchLayer:getRightOutPos(inPos)
