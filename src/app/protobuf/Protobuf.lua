...
function encode( message, t , func , ...)
    --release_print("xxxxxiiixxxxxx11111111")
	...
	if func then
		...
		local s,len = c._wmessage_buffer_string(encoder)
		c._wmessage_delete(encoder)
		return s,len
	end
    ---release_print("xxxxxiiixxxxxx22222222")
end
...
function decode(typename, buffer, length)
	local ret = {}
	local ok,datalen = c._decode(P, decode_message_cb , ret , typename, buffer, length)
	if ok then
		return setmetatable(ret , default_table(typename)),datalen
	else
...
function register_file(filename)
    ...
    local buffer = b.readFile(filename)
    if buffer == nil or buffer == "" then
    	--print("registFail fileName=",filename)
        return
    end
   -- print("filename protobuff=",filename)
    c._env_register(P, buffer)
    return buffer
end
