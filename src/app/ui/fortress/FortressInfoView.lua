...
FortressInfoView = class("FortressInfoView")

--构造
--parent 父结点UI
function FortressInfoView:ctor(parent)
	self.parent = parent
	self:init()
end

--初始化
function FortressInfoView:init()
	--士兵信息面板
...
--创建滑动条
--返回值(无)
function FortressInfoView:createSlider()
    sliderResPath =
    {
...
--设置滑动条数值
--number 数值
function FortressInfoView:setSliderValue(number)
...
--eventType 事件类型
--返回值(无)
function FortressInfoView:fireSoldierBtnCallBack(sender,eventType)
...
--发送解雇士兵请求
function FortressInfoView:sendFireSoldierReq()
	local arms = {}
...
--eventType 事件类型
--返回值(无)
function FortressInfoView:fireBtnCallBack(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
...
--eventType 事件类型
--返回值(无)
function FortressInfoView:closeBtnCallBack(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
...
--更新UI
--info 信息
function FortressInfoView:updateUI(info)
	...
	self.castLab:setString(CommonStr.CASTPOWER .. ":  " .. 0)
