...
UsePropAcceMakeSoldierView = class("UsePropAcceMakeSoldierView")

--构造
--parent 父结点UI
function UsePropAcceMakeSoldierView:ctor(parent)
    self.parent = parent
    self.action = action
    self.items = {}
    self:init()
...
--初始化
function UsePropAcceMakeSoldierView:init()
    ...
    self.titleLab:setString(CommonStr.MAKE_SOLDIER_SPEED)
    --时间
    ...
    local timeInfo = TimeInfoData:getInstance():getTimeInfoByBuildingPos(self.parent.data.pos)
...
--改变数量回调
function UsePropAcceMakeSoldierView:changeNumCallback(sender,event)
...
--更新数量
function UsePropAcceMakeSoldierView:updateNum()
...
--sender 按钮本身
--eventType 事件类型
function UsePropAcceMakeSoldierView:minusBtnCallBack(sender,eventType)
...
--sender 按钮本身
--eventType 事件类型
function UsePropAcceMakeSoldierView:plusBtnCallBack(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        self.curNum = self.curNum + 1
...
--创建道具列表
function UsePropAcceMakeSoldierView:createPropList()
    local wide = 500
    local high = 200
...
--选中道具回调
--sender 按钮本身
function UsePropAcceMakeSoldierView:itemBtnCallBack(sender)
    local index = sender:getTag()
    self:selItmeByIndex(index)
end

--选择物品
function UsePropAcceMakeSoldierView:selItmeByIndex(index)
...
--sender 按钮本身
--eventType 事件类型
function UsePropAcceMakeSoldierView:forWardBtnCallBack(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
...
--sender 按钮本身
--eventType 事件类型
function UsePropAcceMakeSoldierView:backwardBtnCallBack(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        local totalPage = self.propList:getPageCount()
        local curPage = self.propList:getCurPageIdx()
        curPage  = curPage + 1
...
--info 数据
--返回值(无)
function UsePropAcceMakeSoldierView:updateTime(info)
...
--sender 按钮本身
--eventType 事件类型
function UsePropAcceMakeSoldierView:accelerationBtnCallBack(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        local timeInfo = TimeInfoData:getInstance():getTimeInfoByBuildingPos(self.parent.data.pos)
        if timeInfo == nil then
            return
        ...
        if self.itemData.speedTime*self.curNum > leftTime then
            UIMgr:getInstance():openUI(UITYPE.PROP_BOX, {type=PROP_BOX_TYPE.COMMON,text=CommonStr.ACCE_TIME_OVER_MAKESOLDIER_TIME,
...
--发送加速请求
function UsePropAcceMakeSoldierView:sendAcceReq()
    local buildingPos = self.parent.data.pos
    UsePropAcceMakeSoldierService:sendPropAccelerationMakeSoldierReq(self.itemData.templateId,self.itemData.objId,self.curNum,buildingPos)
end

--关闭按钮回调
--sender 按钮本身
--eventType 事件类型
function UsePropAcceMakeSoldierView:closeBtnCallBack(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
