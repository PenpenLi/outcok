...
UsePropAcceBuildingUpLvView = class("UsePropAcceBuildingUpLvView")

--构造
--parent 父结点UI
function UsePropAcceBuildingUpLvView:ctor(parent,action)
	self.parent = parent
    self.action = action
    self.items = {}
	self:init()
...
--初始化
function UsePropAcceBuildingUpLvView:init()
    ...
    local timeInfo = CityBuildingModel:getInstance():getBuilderInfo(self.parent.data.pos)
    if timeInfo == nil then
        if self.action == UpLvPropActionType.create then
            Prop:getInstance():showMsg(CommonStr.BUILDING_NO_NEED_SPEED)
        elseif self.action == UpLvPropActionType.remove then
            Prop:getInstance():showMsg(CommonStr.REMOVE_NO_NEED_SPEED)
        end
        return
    end

    local leftTime = CityBuildingModel:getInstance():getBuildingLeftProcessTime(self.parent.data.pos)
    self.timeLab:setString(CommonStr.BUILD_TIME .. ": " .. Common:getFormatTime(leftTime))
...
--改变数量回调
function UsePropAcceBuildingUpLvView:changeNumCallback(sender,event)
...
--更新数量
function UsePropAcceBuildingUpLvView:updateNum()
...
--sender 按钮本身
--eventType 事件类型
function UsePropAcceBuildingUpLvView:minusBtnCallBack(sender,eventType)
...
--sender 按钮本身
--eventType 事件类型
function UsePropAcceBuildingUpLvView:plusBtnCallBack(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        self.curNum = self.curNum + 1
...
--创建道具列表
function UsePropAcceBuildingUpLvView:createPropList()
    local wide = 500
    local high = 200
...
--sender 按钮本身
--eventType 事件类型
function UsePropAcceBuildingUpLvView:itemBtnCallBack(sender)
    local index = sender:getTag()
    self:selItmeByIndex(index)
end

--选择物品
function UsePropAcceBuildingUpLvView:selItmeByIndex(index)
...
--sender 按钮本身
--eventType 事件类型
function UsePropAcceBuildingUpLvView:forWardBtnCallBack(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
...
--sender 按钮本身
--eventType 事件类型
function UsePropAcceBuildingUpLvView:backwardBtnCallBack(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        local totalPage = self.propList:getPageCount()
        local curPage = self.propList:getCurPageIdx()
        curPage  = curPage + 1
...
--info 数据
--返回值(无)
function UsePropAcceBuildingUpLvView:updateTime(info)
...
--sender 按钮本身
--eventType 事件类型
function UsePropAcceBuildingUpLvView:accelerationBtnCallBack(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        local leftTime = CityBuildingModel:getInstance():getBuildingLeftProcessTime(self.parent.data.pos)
...
--发送加速请求
function UsePropAcceBuildingUpLvView:sendAcceReq()
    local buildingPos = self.parent.data.pos
    local buildingType = CityBuildingModel:getInstance():getBuildType(buildingPos)
    UsePropAcceBuildingUpLvService:sendPropAccelerationBuildingUpLvReq(self.itemData.templateId,self.itemData.objId,self.curNum,buildingPos,buildingType,self.action)
end

--关闭按钮回调
--sender 按钮本身
--eventType 事件类型
function UsePropAcceBuildingUpLvView:closeBtnCallBack(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
