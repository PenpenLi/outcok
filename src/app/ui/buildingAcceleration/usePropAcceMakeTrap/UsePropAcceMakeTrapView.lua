...
UsePropAcceMakeTrapView = class("UsePropAcceMakeTrapView")

--构造
--parent 父结点UI
function UsePropAcceMakeTrapView:ctor(parent)
    self.parent = parent
    self.action = action
    self.items = {}
    self:init()
...
--初始化
function UsePropAcceMakeTrapView:init()
    ...
    self.titleLab:setString(CommonStr.MAKE_TRAP_SPEED)
    --时间
...
--改变数量回调
function UsePropAcceMakeTrapView:changeNumCallback(sender,event)
...
--更新数量
function UsePropAcceMakeTrapView:updateNum()
...
--sender 按钮本身
--eventType 事件类型
function UsePropAcceMakeTrapView:minusBtnCallBack(sender,eventType)
...
--sender 按钮本身
--eventType 事件类型
function UsePropAcceMakeTrapView:plusBtnCallBack(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        self.curNum = self.curNum + 1
...
--创建道具列表
function UsePropAcceMakeTrapView:createPropList()
    local wide = 500
    local high = 200
...
--sender 按钮本身
--eventType 事件类型
function UsePropAcceMakeTrapView:itemBtnCallBack(sender)
    local index = sender:getTag()
    self:selItmeByIndex(index)
end

--选择物品
function UsePropAcceMakeTrapView:selItmeByIndex(index)
...
--sender 按钮本身
--eventType 事件类型
function UsePropAcceMakeTrapView:forWardBtnCallBack(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
...
--sender 按钮本身
--eventType 事件类型
function UsePropAcceMakeTrapView:backwardBtnCallBack(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        local totalPage = self.propList:getPageCount()
        local curPage = self.propList:getCurPageIdx()
        curPage  = curPage + 1
...
--info 数据
--返回值(无)
function UsePropAcceMakeTrapView:updateTime(info)
...
--sender 按钮本身
--eventType 事件类型
function UsePropAcceMakeTrapView:accelerationBtnCallBack(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        local timeInfo = TimeInfoData:getInstance():getTimeInfoByBuildingPos(self.parent.data.pos)
        if timeInfo == nil then
            return
        ...
        if self.itemData.speedTime*self.curNum > leftTime then
            UIMgr:getInstance():openUI(UITYPE.PROP_BOX, {type=PROP_BOX_TYPE.COMMON,text=CommonStr.ACCE_TIME_OVER_MAKETRAP_TIME,
...
--发送加速请求
function UsePropAcceMakeTrapView:sendAcceReq()
    local buildingPos = self.parent.data.pos
    UsePropAcceMakeTrapService:sendAcceReq(self.itemData.templateId,self.itemData.objId,self.curNum,buildingPos)
end

--关闭按钮回调
--sender 按钮本身
--eventType 事件类型
function UsePropAcceMakeTrapView:closeBtnCallBack(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
