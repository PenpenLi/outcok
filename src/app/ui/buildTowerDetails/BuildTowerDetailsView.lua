...
BuildTowerDetailsView = class("BuildTowerDetailsView",UIBaseView)

--构造
--uiType UI类型
--data 数据
--返回值(无)
function BuildTowerDetailsView:ctor(uiType,data)
    self.data = data
...
--初始化
--返回值(无)
function BuildTowerDetailsView:init()
    self.root = Common:loadUIJson(BUILDE_DETAILS_HEAD)
    self:addChild(self.root)

    --关闭按钮
    self.closeBtn = Common:seekNodeByName(self.root,"closeBtn")
    self.closeBtn:addTouchEventListener(handler(self,BuildTowerDetailsView.closeCallback))
    ...
    self.moreInfoBtn:addTouchEventListener(handler(self,self.moreInfoBtnCallback))
    self.moreInfo2Btn = Common:seekNodeByName(self.root,"info2Btn")
    self.moreInfo2Btn:addTouchEventListener(handler(self,self.moreInfoBtnCallback))

     --拆除建筑按钮
    self.demolitionBtn = Common:seekNodeByName(self.root,"demolitionBtn")
    self.demolitionBtn:addTouchEventListener(handler(self,self.demolitionBtnCallback))
...
--UI加到舞台后会调用这个接口
--返回值(无)
function BuildTowerDetailsView:onEnter()
    ...
    if buildingInfo ~= nil then
        ...
        local effect = TowerEffectConfig:getInstance():getConfigInfo(buildingLv)
        if effect ~= nil then
            self.condition1Lab:setString(CommonStr.ATTACK_POWER .. "                                           " ..effect.te_attack)
        else
...
--获取建筑物位置
--返回值(建筑位置)
function BuildTowerDetailsView:getBuildingPos()
...
--eventType 事件类型
--返回值(无)
function BuildTowerDetailsView:demolitionBtnCallback(sender,eventType)
...
--发送移除建筑请求
--返回值(无)
function BuildTowerDetailsView:sendRemoveReq()
...
--eventType 事件类型
--返回值(无)
function BuildTowerDetailsView:moreInfoBtnCallback(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
...
--显示城外建筑详情按钮
--返回值(无)
function BuildTowerDetailsView:displayRemovalButton()
...
--返回值(无)
function BuildTowerDetailsView:setSomeImgVisible(visible)
...
--创建更多信息列表
--返回值(无)
function BuildTowerDetailsView:createMoreInfoList()
    --背景图片
    ...
    local buildingType = CityBuildingModel:getInstance():getBuildType(buildingPos)
    local buildingLv = CityBuildingModel:getInstance():getBuildingLv(buildingPos)
    local detailsInfoList = TowerEffectConfig:getInstance():getDetailsInfo(buildingType)
    ...
    local pos2 = cc.p(310,18)
    local effectLab = display.newTTFLabel({
        text = CommonStr.ATTACK_POWER,
        font = "Arial",
        size = 24,
        color = cc.c3b(255, 0, 0),
        align = cc.TEXT_ALIGNMENT_LEFT,
    ...
    for i=1,#detailsInfoList do
        ...
        local trapCapacityLab = display.newTTFLabel({
            text = detailsInfoList[i].te_attack,
            font = "Arial",
            size = 24,
            color = cc.c3b(255, 0, 0),
            align = cc.TEXT_ALIGNMENT_LEFT,
...
--UI离开舞台后会调用这个接口
--返回值(无)
function BuildTowerDetailsView:onExit()
...
--析构函数(此时已经从内存中释放掉资源了)
--返回值(无)
function BuildTowerDetailsView:onDestroy()
...
--eventType 事件类型
--返回值(无)
function BuildTowerDetailsView:closeCallback(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        MyLog("close click...")
