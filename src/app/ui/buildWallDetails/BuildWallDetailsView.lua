...
BuildWallDetailsView = class("BuildWallDetailsView",UIBaseView)

--构造
--uiType UI类型
--data 数据
--返回值(无)
function BuildWallDetailsView:ctor(uiType,data)
    self.data = data
...
--初始化
--返回值(无)
function BuildWallDetailsView:init()
    self.root = Common:loadUIJson(BUILDE_DETAILS_HEAD)
    self:addChild(self.root)

    --关闭按钮
    self.closeBtn = Common:seekNodeByName(self.root,"closeBtn")
    self.closeBtn:addTouchEventListener(handler(self,BuildWallDetailsView.closeCallback))
    ...
    self.moreInfoBtn:addTouchEventListener(handler(self,BuildWallDetailsView.moreInfoBtnCallback))
...
--UI加到舞台后会调用这个接口
--返回值(无)
function BuildWallDetailsView:onEnter()
    ...
    if buildingInfo ~= nil then
        ...
        local effect = WallEffectConfig:getInstance():getConfigInfo(buildingLv)
        if effect ~= nil then
            self.condition1Lab:setString(CommonStr.TRAP_CAPACITY .. "                                           " ..effect.we_defend)
            self.condition2Lab:setString(CommonStr.DEFENSIVE_POWER .. "                                              " ..effect.we_trap)
        else
...
--获取建筑物位置
--返回值(建筑位置)
function BuildWallDetailsView:getBuildingPos()
...
--eventType 事件类型
--返回值(无)
function BuildWallDetailsView:moreInfoBtnCallback(sender,eventType)
...
--显示城外建筑详情按钮
--返回值(无)
function BuildWallDetailsView:displayRemovalButton()
...
--返回值(无)
function BuildWallDetailsView:setSomeImgVisible(visible)
...
--创建更多信息列表
--返回值(无)
function BuildWallDetailsView:createMoreInfoList()
    --背景图片
    ...
    local buildingType = CityBuildingModel:getInstance():getBuildType(buildingPos)
    local buildingLv = CityBuildingModel:getInstance():getBuildingLv(buildingPos)
    local detailsInfoList = WallEffectConfig:getInstance():getDetailsInfo(buildingType)
    ...
    local battleLab = display.newTTFLabel({
        text = CommonStr.DEFENSIVE_POWER,
        font = "Arial",
        size = 24,
        color = cc.c3b(255, 0, 0),
        align = cc.TEXT_ALIGNMENT_LEFT,
    ...
    for i=1,#detailsInfoList do
        ...
        --当前多少级
        if detailsInfoList[i].we_level == buildingLv then
        ...
        local lvLab = display.newTTFLabel({
            text = "" .. detailsInfoList[i].we_level,
            font = "Arial",
            size = 24,
            color = cc.c3b(255, 0, 0),
            align = cc.TEXT_ALIGNMENT_LEFT,
        ...
        local trapCapacityLab = display.newTTFLabel({
            text = detailsInfoList[i].we_trap,
            font = "Arial",
            size = 24,
            color = cc.c3b(255, 0, 0),
            align = cc.TEXT_ALIGNMENT_LEFT,
        })
        rowBgImg:addChild(trapCapacityLab)
        trapCapacityLab:setAnchorPoint(0.5,0.5)
        trapCapacityLab:setPosition(pos2.x+30,pos2.y)

        local defenseLab = display.newTTFLabel({
            text = "" .. detailsInfoList[i].we_defend,
            font = "Arial",
            size = 24,
            color = cc.c3b(255, 0, 0),
            align = cc.TEXT_ALIGNMENT_LEFT,
        })
        rowBgImg:addChild(defenseLab)
        defenseLab:setAnchorPoint(0.5,0.5)
        defenseLab:setPosition(pos3.x+30,pos3.y)
        ...
        battleLab:setPosition(pos4.x+30,pos4.y)
    end
...
--UI离开舞台后会调用这个接口
--返回值(无)
function BuildWallDetailsView:onExit()
...
--析构函数(此时已经从内存中释放掉资源了)
--返回值(无)
function BuildWallDetailsView:onDestroy()
...
--eventType 事件类型
--返回值(无)
function BuildWallDetailsView:closeCallback(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        MyLog("close click...")
