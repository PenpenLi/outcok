...
BuildingMarchDetailsView = class("BuildingMarchDetailsView",UIBaseView)

--构造
--uiType UI类型
--data 数据
--返回值(无)
function BuildingMarchDetailsView:ctor(uiType,data)
    self.data = data
...
--初始化
--返回值(无)
function BuildingMarchDetailsView:init()
    ...
    self.closeBtn:addTouchEventListener(handler(self,BuildingMarchDetailsView.closeCallback))
    ...
    self.demolitionBtn:addTouchEventListener(handler(self,BuildingMarchDetailsView.demolitionBtnCallback))

    --更多信息按钮
    self.moreInfoBtn = Common:seekNodeByName(self.root,"infoBtn")
    self.moreInfo2Btn = Common:seekNodeByName(self.root,"info2Btn")
    self.moreInfo2Btn:addTouchEventListener(handler(self,BuildingMarchDetailsView.moreInfoBtnCallback))
    self.moreInfoBtn:addTouchEventListener(handler(self,BuildingMarchDetailsView.moreInfoBtnCallback))
...
--UI加到舞台后会调用这个接口
--返回值(无)
function BuildingMarchDetailsView:onEnter()
    ...
    --资源描述
    if buildingInfo ~= nil then
        local allSoldier = BuildingMarchDetailsModel:getInstance():getAllSoldier(buildingType)
        local campEffec = CampEffectConfig:getInstance():getConfigInfo(buildingInfo.level)
        if campEffec ~= nil then
            self.condition1Lab:setString(CommonStr.TRAINING_QUANTITY ..  "   " .. campEffec.ce_soldier)
            self.condition2Lab:setString(CommonStr.TRAINING_SPEED .. "  +" .. campEffec.ce_speed*100 .. "%")
            self.condition3Lab:setString(CommonStr.ALL_TRAINING_QUANTITY ..  "   " .. allSoldier)
        end
    end
end
...
--获取建筑物位置
--返回值(建筑位置)
function BuildingMarchDetailsView:getBuildingPos()
...
--eventType 事件类型
--返回值(无)
function BuildingMarchDetailsView:moreInfoBtnCallback(sender,eventType)
...
--eventType 事件类型
--返回值(无)
function BuildingMarchDetailsView:demolitionBtnCallback(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        local buildingPos = self:getBuildingPos()
        local buildingInfo = CityBuildingModel:getInstance():getBuildInfo(buildingPos)
        local buildingType = CityBuildingModel:getInstance():getBuildType(buildingPos)
...
--发送移除建筑请求
--返回值(无)
function BuildingMarchDetailsView:sendRemoveReq()
    CityBuildingService:removeBuildingReq(self:getBuildingPos())
end

--显示城外建筑详情按钮
--返回值(无)
function BuildingMarchDetailsView:displayRemovalButton()
...
--返回值(无)
function BuildingMarchDetailsView:setSomeImgVisible(visible)
...
--创建更多信息列表
--返回值(无)
function BuildingMarchDetailsView:createMoreInfoList()
    --背景图片
    ...
    local effectLab = display.newTTFLabel({
        text = CommonStr.TRAINING_QUANTITY,
        font = "Arial",
        size = 24,
        color = cc.c3b(255, 0, 0),
        align = cc.TEXT_ALIGNMENT_LEFT,
    ...
    local detailsInfoList = CampEffectConfig:getInstance():getDetailsInfo()
    ...
    for i=1,#detailsInfoList do
        ...
        if detailsInfoList[i].ce_lv == buildingLv then
        ...
        local soldier = detailsInfoList[i].ce_soldier or " "
        local soldierLab = display.newTTFLabel({
            text = detailsInfoList[i].ce_soldier,
            font = "Arial",
            size = 24,
            color = cc.c3b(255, 0, 0),
            align = cc.TEXT_ALIGNMENT_LEFT,
        })
        rowBgImg:addChild(soldierLab)
        soldierLab:setAnchorPoint(0,0.5)
        soldierLab:setPosition(pos2.x+20,pos2.y)

        local speed = detailsInfoList[i].ce_speed or " "
        local speedLab = display.newTTFLabel({
            text = "+" .. detailsInfoList[i].ce_speed*100 .. "%",
            font = "Arial",
            size = 24,
            color = cc.c3b(255, 0, 0),
            align = cc.TEXT_ALIGNMENT_LEFT,
        })
        rowBgImg:addChild(speedLab)
        speedLab:setAnchorPoint(0,0.5)
        speedLab:setPosition(pos3.x+20,pos3.y)
...
--UI离开舞台后会调用这个接口
--返回值(无)
function BuildingMarchDetailsView:onExit()
    UICommon:getInstance():setMapTouchAable(true)
end

--析构函数(此时已经从内存中释放掉资源了)
--返回值(无)
function BuildingMarchDetailsView:onDestroy()
...
--eventType 事件类型
--返回值(无)
function BuildingMarchDetailsView:closeCallback(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        MyLog("close click...")
