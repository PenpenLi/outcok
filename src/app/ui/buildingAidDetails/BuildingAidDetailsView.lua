...
BuildingAidDetailsView = class("BuildingAidDetailsView",UIBaseView)

--构造
--uiType UI类型
--data 数据
--返回值(无)
function BuildingAidDetailsView:ctor(uiType,data)
    self.data = data
...
--初始化
--返回值(无)
function BuildingAidDetailsView:init()
    ...
    self.closeBtn:addTouchEventListener(handler(self,BuildingAidDetailsView.closeCallback))
    ...
    self.demolitionBtn:addTouchEventListener(handler(self,BuildingAidDetailsView.demolitionBtnCallback))

    --更多信息按钮
    self.moreInfoBtn = Common:seekNodeByName(self.root,"infoBtn")
    self.moreInfo2Btn = Common:seekNodeByName(self.root,"info2Btn")
    self.moreInfo2Btn:addTouchEventListener(handler(self,BuildingAidDetailsView.moreInfoBtnCallback))
    self.moreInfoBtn:addTouchEventListener(handler(self,BuildingAidDetailsView.moreInfoBtnCallback))
...
--UI加到舞台后会调用这个接口
--返回值(无)
function BuildingAidDetailsView:onEnter()
    --MyLog("BuildingAidDetailsView onEnter...")ing
    ...
    --资源描述
    if buildingInfo ~= nil then
        local aidEffect = AidEffectConfig:getInstance():getConfigInfo(buildingInfo.level)
        local allCapacity = self:getAllCapacity()
        local allTurbo = self:getAllTurbo()
        if aidEffect ~= nil then
            self.condition1Lab:setString(CommonStr.HERO_TREATMENT_SPEED ..  "      +" .. aidEffect.ae_turbo*100 .. "%")
            self.condition2Lab:setString(CommonStr.ALL_HERO_TREATMENT_SPEED ..  "  +" .. allTurbo*100 .. "%")
            self.condition3Lab:setString(CommonStr.CAPACITY_WOUNDED .. "          " .. aidEffect.ae_capacity)
            self.condition4Lab:setString(CommonStr.ALL_CAPACITY_WOUNDED ..  "      " .. allCapacity)
        end
    end
end
...
--获取总伤兵容量
--返回值()
function BuildingAidDetailsView:getAllCapacity()
    local buildingPos = self:getBuildingPos()
    local buildingType = CityBuildingModel:getInstance():getBuildType(buildingPos)
    local num = 0
...
--获取总治疗速度
--返回值()
function BuildingAidDetailsView:getAllTurbo()
    local buildingPos = self:getBuildingPos()
    local buildingType = CityBuildingModel:getInstance():getBuildType(buildingPos)
    local num = 0
    local arr = CityBuildingModel:getInstance():getBuildListByType(buildingType)
...
--获取建筑物位置
--返回值(建筑位置)
function BuildingAidDetailsView:getBuildingPos()
...
--eventType 事件类型
--返回值(无)
function BuildingAidDetailsView:moreInfoBtnCallback(sender,eventType)
...
--eventType 事件类型
--返回值(无)
function BuildingAidDetailsView:demolitionBtnCallback(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        local buildingPos = self:getBuildingPos()
        local buildingInfo = CityBuildingModel:getInstance():getBuildInfo(buildingPos)
        local buildingType = CityBuildingModel:getInstance():getBuildType(buildingPos)
...
--发送移除建筑请求
--返回值(无)
function BuildingAidDetailsView:sendRemoveReq()
    CityBuildingService:removeBuildingReq(self:getBuildingPos())
end

--显示城外建筑详情按钮
--返回值(无)
function BuildingAidDetailsView:displayRemovalButton()
...
--返回值(无)
function BuildingAidDetailsView:setSomeImgVisible(visible)
...
--创建更多信息列表
--返回值(无)
function BuildingAidDetailsView:createMoreInfoList()
    --背景图片
    ...
    local effectLab = display.newTTFLabel({
        text = CommonStr.HERO_TREATMENT_SPEED,
        font = "Arial",
        size = 24,
        color = cc.c3b(255, 0, 0),
        align = cc.TEXT_ALIGNMENT_LEFT,
    ...
    local battleLab = display.newTTFLabel({
        text = CommonStr.CAPACITY_WOUNDED,
        font = "Arial",
        size = 24,
        color = cc.c3b(255, 0, 0),
        align = cc.TEXT_ALIGNMENT_LEFT,
    ...
    local detailsInfoList = AidEffectConfig:getInstance():getDetailsInfo()
    for i=1,#detailsInfoList do
        ...
        if detailsInfoList[i].ae_level == buildingLv then
        ...
        local lvLab = display.newTTFLabel({
            text = "" .. detailsInfoList[i].ae_level,
            font = "Arial",
            size = 24,
            color = cc.c3b(255, 0, 0),
            align = cc.TEXT_ALIGNMENT_LEFT,
        ...
        local turbo = detailsInfoList[i].ae_turbo or " "
        local turboLab = display.newTTFLabel({
            text = "+" .. detailsInfoList[i].ae_turbo*100 .. "%",
            font = "Arial",
            size = 24,
            color = cc.c3b(255, 0, 0),
            align = cc.TEXT_ALIGNMENT_LEFT,
        ...
        turboLab:setPosition(pos2.x+40,pos2.y)

        local capacity = detailsInfoList[i].ae_capacity or " "
        local capacityLab = display.newTTFLabel({
            text = "" .. detailsInfoList[i].ae_capacity,
            font = "Arial",
            size = 24,
            color = cc.c3b(255, 0, 0),
            align = cc.TEXT_ALIGNMENT_LEFT,
        ...
        local fight = detailsInfoList[i].ae_fight or " "
        local fightLab = display.newTTFLabel({
            text = "" .. detailsInfoList[i].ae_fight,
            font = "Arial",
            size = 24,
            color = cc.c3b(255, 0, 0),
            align = cc.TEXT_ALIGNMENT_LEFT,
...
--UI离开舞台后会调用这个接口
--返回值(无)
function BuildingAidDetailsView:onExit()
    UICommon:getInstance():setMapTouchAable(true)
end

--析构函数(此时已经从内存中释放掉资源了)
--返回值(无)
function BuildingAidDetailsView:onDestroy()
    --MyLog("BuildingAidDetailsView:onDestroy")
end
...
--eventType 事件类型
--返回值(无)
function BuildingAidDetailsView:closeCallback(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        MyLog("close click...")
