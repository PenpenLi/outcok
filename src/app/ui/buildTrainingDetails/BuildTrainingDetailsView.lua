...
BuildTrainingDetailsView = class("BuildTrainingDetailsView",UIBaseView)

--构造
--uiType UI类型
--data 数据
--返回值(无)
function BuildTrainingDetailsView:ctor(uiType,data)
    self.data = data
...
--初始化
--返回值(无)
function BuildTrainingDetailsView:init()
    ...
    self.closeBtn:addTouchEventListener(handler(self,BuildTrainingDetailsView.closeCallback))
    ...
    self.demolitionBtn:addTouchEventListener(handler(self,BuildTrainingDetailsView.demolitionBtnCallback))

    --更多信息按钮
    self.moreInfoBtn = Common:seekNodeByName(self.root,"infoBtn")
    self.moreInfo2Btn = Common:seekNodeByName(self.root,"info2Btn")
    self.moreInfo2Btn:addTouchEventListener(handler(self,BuildTrainingDetailsView.moreInfoBtnCallback))
    self.moreInfoBtn:addTouchEventListener(handler(self,BuildTrainingDetailsView.moreInfoBtnCallback))
...
--UI加到舞台后会调用这个接口
--返回值(无)
function BuildTrainingDetailsView:onEnter()
...
--获取建筑物位置
--返回值(建筑位置)
function BuildTrainingDetailsView:getBuildingPos()
...
--eventType 事件类型
--返回值(无)
function BuildTrainingDetailsView:moreInfoBtnCallback(sender,eventType)
...
--eventType 事件类型
--返回值(无)
function BuildTrainingDetailsView:demolitionBtnCallback(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        local buildingPos = self:getBuildingPos()
        local buildingInfo = CityBuildingModel:getInstance():getBuildInfo(buildingPos)
        local buildingType = CityBuildingModel:getInstance():getBuildType(buildingPos)
...
--发送移除建筑请求
--返回值(无)
function BuildTrainingDetailsView:sendRemoveReq()
    CityBuildingService:removeBuildingReq(self:getBuildingPos())
end

--显示城外建筑详情按钮
--返回值(无)
function BuildTrainingDetailsView:displayRemovalButton()
...
--返回值(无)
function BuildTrainingDetailsView:setSomeImgVisible(visible)
...
--创建更多信息列表
--返回值(无)
function BuildTrainingDetailsView:createMoreInfoList()
    --背景图片
    ...
    local effectLab = display.newTTFLabel({
        text = CommonStr.TRAINING_POSITION,
        font = "Arial",
        size = 24,
        color = cc.c3b(255, 0, 0),
        align = cc.TEXT_ALIGNMENT_LEFT,
    ...
    local battleLab = display.newTTFLabel({
        text = CommonStr.EXPERIENCE_HOURS,
        font = "Arial",
        size = 24,
        color = cc.c3b(255, 0, 0),
        align = cc.TEXT_ALIGNMENT_LEFT,
    ...
    local detailsInfoList = TrainEffectConfig:getInstance():getTrainConfigInfo(buildingType)
    for i=1,#detailsInfoList do
        ...
        if detailsInfoList[i].te_level == buildingLv then
        ...
        local lvLab = display.newTTFLabel({
            text = "" .. detailsInfoList[i].te_level,
            font = "Arial",
            size = 24,
            color = cc.c3b(255, 0, 0),
            align = cc.TEXT_ALIGNMENT_LEFT,
        ...
        local yieldsLab = display.newTTFLabel({
            text = detailsInfoList[i].te_trainnum,
            font = "Arial",
            size = 24,
            color = cc.c3b(255, 0, 0),
            align = cc.TEXT_ALIGNMENT_LEFT,
        ...
        yieldsLab:setAnchorPoint(0.5,0.5)
        yieldsLab:setPosition(pos2.x+40,pos2.y)

        local capacityLab = display.newTTFLabel({
            text = "" .. detailsInfoList[i].te_exp,
            font = "Arial",
            size = 24,
            color = cc.c3b(255, 0, 0),
            align = cc.TEXT_ALIGNMENT_LEFT,
        ...
        capacityLab:setAnchorPoint(0.5,0.5)
        capacityLab:setPosition(pos3.x+50,pos3.y)
        ...
        fightforceLab:setAnchorPoint(0.5,0.5)
        fightforceLab:setPosition(pos4.x+30,pos4.y)
    end
...
--UI离开舞台后会调用这个接口
--返回值(无)
function BuildTrainingDetailsView:onExit()
    UICommon:getInstance():setMapTouchAable(true)
end

--析构函数(此时已经从内存中释放掉资源了)
--返回值(无)
function BuildTrainingDetailsView:onDestroy()
    --MyLog("BuildTrainingDetailsView:onDestroy")
end
...
--eventType 事件类型
--返回值(无)
function BuildTrainingDetailsView:closeCallback(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        MyLog("close click...")
