...
BuildingUpLvView = class("BuildingUpLvView",UIBaseView)

--构造
--uiType UI类型
--data 数据
--返回值(无)
function BuildingUpLvView:ctor(uiType,data)
    self.data = data
...
--初始化
--返回值(无)
function BuildingUpLvView:init()
    ...
    self.closeBtn:addTouchEventListener(handler(self,BuildingUpLvView.closeCallback))
...
--eventType 事件类型
--返回值(无)
function BuildingUpLvView:rightNowUpLvBtnCallback(sender,eventType)
...
--是否可以升级建筑
function BuildingUpLvView:isCanUpBuilding()
    ...
    --粮食判断
    if info.bu_grain > playerData:getFood() then
        print("粮食不足")
        Prop:getInstance():showMsg(CommonStr.NOT_ENOUGH_FOOD)
        return
    end
    --木头判断
    if info.bu_wood > playerData.wood then
        print("木头不足")
        Prop:getInstance():showMsg(CommonStr.NOT_ENOUGH_WOOD)
        return
    end
    --铁矿判断
    if info.bu_iron > playerData.iron then
        print("铁矿不足")
        Prop:getInstance():showMsg(CommonStr.NOT_ENOUGH_IRON)
        return
    end
    --秘银判断
    if info.bu_mithril > playerData.mithril then
        print("秘银不足")
...
--发送加速建筑升级请求
--返回值(无)
function BuildingUpLvView:sendAccelerationUpLvReq()
...
--获取建筑物位置
--返回值(建筑位置)
function BuildingUpLvView:getBuildingPos()
    return self.data.building:getTag()
end

--获取建筑升级时间
--返回值(升级时间)
function BuildingUpLvView:getBuildingUpTime()
...
--创建升级建筑条件列表
--返回值(无)
function BuildingUpLvView:createUpBuildingConditionList()
...
--eventType 事件类型
--返回值(无)
function BuildingUpLvView:upgradeInfoBtnCallback(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        if not self:isCanUpBuilding() then
            return
        end
...
--UI加到舞台后会调用这个接口
--返回值(无)
function BuildingUpLvView:onEnter()
...
--UI离开舞台后会调用这个接口
--返回值(无)
function BuildingUpLvView:onExit()
...
--析构函数(此时已经从内存中释放掉资源了)
--返回值(无)
function BuildingUpLvView:onDestroy()
...
--eventType 事件类型
--返回值(无)
function BuildingUpLvView:closeCallback(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        MyLog("close click...")
